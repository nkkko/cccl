...
private:
  ...
  _CCCL_HIDE_FROM_ABI static pointer __copy_same(const_pointer __first, const_pointer __last, pointer __dest)
  {
  ...
  _CCCL_HIDE_FROM_ABI static void __copy_cross(_Iter __first, _Iter __last, pointer __dest, size_type __count)
  {
  ...
  _CCCL_HIDE_FROM_ABI void __create_gap(size_type __pos, size_type __count)
  {
    ...
    { // need to reallocate
      __buffer __old_buf = __buf_.__replace_allocation(__new_size);
  ...
  //! @note This does *not* change the size of the vector and should be followed by a call to erase
...
public:
  ...
  _CCCL_HIDE_FROM_ABI vector(const vector& __other)
  ...
  _CCCL_HIDE_FROM_ABI vector(vector&& __other) noexcept
  ...
  _CCCL_HIDE_FROM_ABI explicit vector(const vector<_Tp, _OtherProperties...>& __other)
      : __buf_(__other.get_resource(), __other.__size_)
      , __size_(__other.__size_)
  {
    if (__other.__size_ != 0)
  ...
  _LIBCUDACXX_TEMPLATE(class... _OtherProperties)
  _LIBCUDACXX_REQUIRES(__properties_match<_OtherProperties...>)
  _CCCL_HIDE_FROM_ABI explicit vector(vector<_Tp, _OtherProperties...>&& __other) noexcept
      : __buf_(_CUDA_VSTD::move(__other.__buf_))
      , __size_(_CUDA_VSTD::exchange(__other.__size_, 0))
  {}
  ...
  _CCCL_HIDE_FROM_ABI vector(__resource __mr)
  ...
  _CCCL_HIDE_FROM_ABI explicit vector(__resource __mr, const size_type __size)
  ...
  //! @note If `__size == 0` then no memory is allocated.
  _CCCL_HIDE_FROM_ABI explicit vector(__resource __mr, const size_type __size, const _Tp& __value)
      : __buf_(_CUDA_VSTD::move(__mr), __size)
      , __size_(__size)
  {
    if (__size != 0)
  ...
  _CCCL_HIDE_FROM_ABI explicit vector(__resource __mr, const size_type __size, ::cuda::experimental::uninit_t)
      : __buf_(_CUDA_VSTD::move(__mr), __size)
      , __size_(__size)
  {}
  ...
  _CCCL_HIDE_FROM_ABI vector(__resource __mr, _Iter __first, _Iter __last)
      : __buf_(_CUDA_VSTD::move(__mr), 0)
  {
  ...
  _CCCL_HIDE_FROM_ABI vector(__resource __mr, _CUDA_VSTD::initializer_list<_Tp> __ilist)
  ...
  _CCCL_HIDE_FROM_ABI vector(__resource __mr, _Range&& __range)
      : __buf_(_CUDA_VSTD::move(__mr), 0)
  {
  ...
  _CCCL_HIDE_FROM_ABI vector& operator=(const vector& __other)
  {
    ...
    {
      __buffer __new_buf{__other.get_resource(), __count};
  ...
  _CCCL_HIDE_FROM_ABI vector& operator=(vector&& __other) noexcept
  {
    if (this == _CUDA_VSTD::addressof(__other))
    {
      return *this;
  ...
  _CCCL_HIDE_FROM_ABI vector& operator=(_CUDA_VSTD::initializer_list<_Tp> __ilist)
  {
  ...
  _LIBCUDACXX_TEMPLATE(class... _OtherProperties)
  _LIBCUDACXX_REQUIRES(__properties_match<_OtherProperties...>)
  _CCCL_HIDE_FROM_ABI vector& operator=(const vector<_Tp, _OtherProperties...>& __other)
  {
    ...
    const auto __count = __other.size();
    if (__borrow_resource() != _CUDA_VMR::resource_ref<_Properties...>(__other.__borrow_resource()))
    {
      __buffer __new_buf{__other.get_resource(), __count};
      _CUDA_VSTD::swap(__buf_, __new_buf);
    }
  ...
  _LIBCUDACXX_TEMPLATE(class... _OtherProperties)
  _LIBCUDACXX_REQUIRES(__properties_match<_OtherProperties...>)
  _CCCL_HIDE_FROM_ABI vector& operator=(vector<_Tp, _OtherProperties...>&& __other)
  {
  ...
  _CCCL_HIDE_FROM_ABI void reserve(const size_type __size) noexcept
  {
    ...
    __buffer __old_buf = __buf_.__replace_allocation(__size);
  ...
  _CCCL_HIDE_FROM_ABI void shrink_to_fit() noexcept
  {
    ...
    __buffer __old_buf = __buf_.__replace_allocation(__size_);
  ...
  //! @param __other The other vector.
  _CCCL_HIDE_FROM_ABI void swap(vector& __other) noexcept
  {
  ...
  _CCCL_HIDE_FROM_ABI friend void swap(vector& __lhs, vector& __rhs) noexcept
  {
    __lhs.swap(__rhs);
  }
  //! @}
  ...
  _CCCL_HIDE_FROM_ABI friend void get_property(const vector&, _Property) noexcept {}
#endif // DOXYGEN_SHOULD_SKIP_THIS
};
...
//! @return The number of elements that have been removed.
template <class _Tp, class... _Properties>
_CCCL_HIDE_FROM_ABI size_t erase(vector<_Tp, _Properties...>& __cont,
...
_CCCL_HIDE_FROM_ABI size_t erase_if(vector<_Tp, _Properties...>& __cont,
...
template <class _Tp>
using host_vector = vector<_Tp, _CUDA_VMR::host_accessible>;
